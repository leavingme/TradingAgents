...
- 核心运行时分析流 `run_analysis_stream` 已集成 `history_store` 包装器。无论是通过 CLI（`tradingagents` 交互式运行）还是 Web 界面启动的分析任务，均会自动向统一的 SQLite 数据库 `~/.tradingagents/webui_runs.db` 中持久化记录运行历史与完整的事件步骤（`events`）。
