...
- paired shadow 还必须先注册绑定两臂实际 architecture fingerprint 的 `architecture-experiment-plan/v1`，并以 1–5 组（默认 2）小型 pilot 验证双臂可用决策、实际 market-data date、Research Manager 分叉前输入和 analysis evidence 完全一致。每个 run 的计划归属只通过有界 `architecture-experiment-membership/v1` operator 事件固化；任一 pilot pair 不可用或不匹配时，后续调度必须返回 `experiment_pilot_failed`/exit 1 并停止新增双倍成本，建议改用共享 pre-treatment snapshot/replay。pilot 通过后也要持续复核新 pair，后续漂移同样立即停损；该控制面不得进入 Agent state 或改变决策 fingerprint。
- 成熟 outcome 的 paired comparison 必须从原始 run 事件安全回读实验成员关系，且两臂必须属于同一 experiment ID、同一预注册计划、匹配各自 evaluation 架构身份，并实际按事件声明的 1/2 顺序启动。缺失、非法、冲突、跨计划或计划顺序未执行的 pair 必须计入 `experiment_membership_mismatches_excluded`；同一查询出现多个计划时必须显式选择计划 SHA-256，否则整个比较 `invalid_comparison`。成员字段只允许在 CLI/API operator 查询边界附加，不得修改 immutable evaluation、进入 Agent state 或改变决策 fingerprint。
