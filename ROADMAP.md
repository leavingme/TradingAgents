...
- [ ] **7. 连续多日评估与 agent 架构实验门禁**
  ...
  - pre-treatment 与实际 `market_data_date` 门禁加入后的架构、纵向、history、runtime、结构化 agent、调度、CLI 与 Web 回归共 232 项通过；正式 SQLite 只读审计此前确认 3 个 `architecture_input_*` 列已迁移，当前仍为 1 个旧失败 run、0 个 evaluation。
