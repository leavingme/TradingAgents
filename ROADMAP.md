...
- [ ] **6. 收盘后每日运行与固定期限结果闭环**
  ...
  - 固定 5-session 结果使用分析日之后第一个标的/基准共同收盘价作为 entry，并只在再往后第 5 个共同交易日存在时结算；1–4 个持有时段保持 pending。`point_in_time` 禁止产生事后反思或结果写入。
  ...
  - scheduler dry-run 现构造与正式执行相同的有效 runtime config，输出 analysts、深度、语言、模型、vendor、推理设置、纵向模式、计划顺序以及安全 canonical manifest/fingerprint；不创建历史、不调用 LLM/vendor，也不输出 backend URL、凭据、secret 环境值或绝对路径。首跑时点预演已确认 NVDA `2026-07-17` 为 `would_run`，使用 4 个 analysts、Shallow=1、Chinese、`minimax-cn/MiniMax-M3`、默认完整 vendor 链、live 模式与 `research_and_portfolio`，正式数据库不会错误拦截；修正计量时间轴后的预期 v2 fingerprint 为 `4e47cea0004cee7fbdaee60ea3d35d5a985b4b2c21a7526a53ba0500de863705`，脱敏扫描未发现绝对工作区路径或 secret key 名。
  - 首跑前结果时间轴审计发现旧实现把 `analysis_date` 当日收盘价作为 entry；对 16:30 收盘后形成且可使用盘后信息的决策，该价格早于 `decision_as_of`，不可执行并会形成前视收益。现统一改为分析日之后第一个共同收盘价入场，再持有 5 个共同交易时段；History Store 拒绝 `entry_date <= analysis_date`。`measurement_version` 独立于评分版本持久化，旧 `decision-close-v1` 与新 `next-common-close-v1` rollup/比较 cohort fail-closed 隔离，纵向注入 schema 升级到 v4；相关 runtime、Web、CLI、调度、memory、history、纵向和结构化 agent 门禁 211 项通过。
