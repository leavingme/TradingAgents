...
- [ ] **7. 连续多日评估与 agent 架构实验门禁**
  ...
  - 正式 SQLite 的 immutable 只读审计确认当前仅有 1 条 NVDA run：`2026-07-17`、`review_required`、fingerprint `5891dc…`；它应继续跨 fingerprint 阻止周末用新 live 信息重复生成同日决策，而 active `812d66…` 等待下一交易日建立首个自然基线。审计同时发现旧 fingerprint 的普通失败也会按 version 混入新 active identity 的两次预算，安全修复若在当天部署仍可能被旧实现阻断。scheduler 现只对 `failed|cancelled|unavailable` 使用 active fingerprint 隔离，旧失败不再继承次数或 retry delay；当前 fingerprint 失败仍有界，legacy/pre-runtime 行及 identity 预览失败保守计数，成功、active、final-bar readiness 与 outcome settlement 状态继续跨 fingerprint 幂等。由此既允许已修复实现完成当天缺失决策，又不能通过源码变更重复成功决策或绕过数据/并发门禁。正式 schedule/Web 配置 + 临时 SQLite 对照 dry-run 证明旧 `review_required` 返回 `already_recorded`，而两个旧 fingerprint 失败已耗尽预算时 active `812d66…` 返回 `would_run`；两边均未调用 vendor/LLM。扩展核心门禁 332 项、全仓离线门禁 919 项与 69 个 subtest 通过，3 项依赖缺失/显式 live probe 跳过，前述 2 个环境用例保持 deselected；scheduler 属纯运维范围，active fingerprint 仍为 `812d66f662c0b1765bbd7e9ff22c7e1d8e7d7ba3f555d039de2764a1170a34ec`。
  - 正式 run 复盘确认 `review_required` 是正确的确定性拒绝：SQLite terminal 为 `NO_DECISION`，原因是 Portfolio Manager 把可执行数字写进 prose。但同一 run 在北京时间 2026-07-18 04:41 完成后，共享的 `results/NVDA/2026-07-17/reports/` 又于 14:42 被 remediation 改写成 Overweight 报告；历史 run 的 Web 路径因此会把另一轮可执行内容冒充已拒绝决策。canonical runtime 现将报告写入 `reports/<run_id>/`，对不安全 run ID 使用稳定 hash 目录，并在 `run_completed` 持久化完整报告 SHA-256；Web 只在 results root 内路径和 hash 同时匹配时返回，legacy 无 hash 或文件变更均 409 fail closed。双 run 同 ticker/date 端到端回归证明路径不同且首份字节不变，持久化 hash 与文件一致，路径穿越不能逃逸，Web 对 missing/mismatch 均拒绝。扩展门禁 340 项、全仓离线门禁 923 项与 69 个 subtest 通过，3 项依赖缺失/显式 live probe 跳过，前述 2 个环境用例保持 deselected；Python 编译、前端模块/语法与 diff 检查通过。正式 schedule/Web 配置 + 隔离 SQLite 的零 vendor/LLM `status` 与 `run --dry-run` 一致得到新 active fingerprint `a3078ecef654c38b8511c0f19cac2ab338a87d3b9072e8f6ff01597842402f89`，dry-run 选择 `2026-07-17` catch-up 且数据库保持 0 run。该安全修复按 manifest scope 重建 identity，优先级高于首个基线连续性。
  - 上述报告修复也暴露了 cohort identity 的边界缺口：报告目录与 hash 只是 decision 后的 operator artifact，却因实现留在 `runtime/analysis_runner.py` 而改变 implementation digest；若继续这样维护，纯展示/存储改动会不断切碎连续多日样本。现将 run-scoped 目录、写入和 SHA-256 封装进 digest scope 外的 `tradingagents.reporting.write_run_report()`，decision runtime 仅保留稳定调用边界。回归在两个等价源码树中证明 `reporting.py` 与 automation 改动不改变 digest，而 `runtime/analysis_runner.py` 或 Agent prompt 改动仍会改变 digest；不可变双 run、路径穿越与持久化 hash 端到端测试继续通过。针对性 architecture/reporting/runtime/Web/daily/active/evaluation/operator 门禁 149 项通过；全仓离线门禁仍为 923 项与 69 个 subtest 通过、3 项依赖缺失/显式 live probe 跳过、2 个已知外网/未隔离用例 deselected。正式 schedule/Web 配置 + 隔离 SQLite 的 `status` 与零 vendor/LLM `run --dry-run` 一致得到 active fingerprint `631a371ef76aed83a3b93f0b681e4ccdf37d765b471d9249aee8a0e5c53a6241`，选择 `2026-07-17` catch-up 且数据库保持 0 run。该结构调整会最后一次重建尚无有效 evaluation 的 active identity，但后续纯报告维护不再切碎正式 Agent cohort。
  - 下一自然运行前的凭据审计确认正式 MCP bearer 仍为 `expired/configured=false`，安全 UTC expiry 为 `2026-07-18T15:56:02.805037+00:00`。新增三条确定性回归不只模拟 router 异常，而是让真实 MCP adapter 加载带时区的过期 token：OHLCV 和结构化个股新闻分别进入配置中的 Longbridge CLI；组合财务能力在 MCP 的 IS/BS/CF/公司资料全部认证失败后，以完整原子能力切换到 CLI，再通过统一 validator、跨表 reconciliation 和紧凑 evidence renderer，未混用半套 vendor。相关 MCP/router/news/financial/runtime/daily/Web 扩展门禁 165 项通过；全仓离线门禁增至 926 项与 69 个 subtest 通过，3 项依赖缺失/显式 live probe 跳过，2 个已知外网/未隔离用例 deselected。本轮只补到期后 fallback 控制流证据，没有修改决策实现，active fingerprint 保持 `631a371ef76aed83a3b93f0b681e4ccdf37d765b471d9249aee8a0e5c53a6241`；真实 CLI OAuth/网络可用性仍必须在下一自然窗口前后单独探测，不能由离线 mock 冒充。
  - 2026-07-19 运行态复核确认 user-systemd `tradingagents-daily.timer` 为 enabled/active，每 15 分钟触发；12:45 CST 的最近 oneshot 以 exit 0 返回同日既有 `review_required`，未重复调用 Graph。`tradingagents-web.service` 连续运行，systemd 展示的 `/data/disk/workspace/TradingAgents` 与文档 `/data/workspace/TradingAgents` 设备号/inode 完全相同，只是宿主挂载别名，不是旧工作树。正式 CLI `status/evaluate` 与运行中 `/api/evaluations?ticker=NVDA` 一致显示 active `631a371e…` 尚无 terminal/evaluation、旧 `5891dc…` 仅有 1 条 `review_required` 成本样本（385658 input tokens / 657.89 秒）、measurement continuity 为 `awaiting_initial_run`，建议 `collect_first_active_run_without_decision_changes` 且 `automatic_architecture_mutation_allowed=false`。运行中 env-status 正确显示 MCP expired、Longbridge CLI configured，但 vendor verification 时间仍在 expiry 前，不能当作到期后 live 证据；旧共享报告 API 实测 409 fail closed。由此当前最佳动作是保持决策 identity，等待下一收盘窗口建立首个自然基线，而不是凭旧 findings 或无 attribution 总量继续改 Agent。
  - Providers 运行态展示已消除上述凭据/历史验证歧义：`/api/config/env-status` 保留不可变的历史 verification `status`，并逐条补充 `historical`、`current_credential_usable` 与 `current_credential_status`；凭据缺失或过期时，Web 主状态显示“当前不可用”、凭据状态显示“凭据已过期”，历史成功只标为“上次验证”，不再把 expiry 前的 `available` 冒充当前可用性。后端、前端与 MCP/router fallback 相关扩展门禁 110 项及前端模块/语法检查通过；全仓离线门禁为 927 项与 69 个 subtest 通过、3 项依赖缺失/显式 live probe 跳过、2 个已知外网/未隔离用例 deselected。运行中 API 已返回 expired + historical available + current credential unusable 的三维一致状态；1440×1000 与 390×844 浏览器审计均显示新文案且无横向 overflow。零 vendor/LLM dry-run 确认该 operator-only 改动未进入 architecture digest，active fingerprint 仍为 `631a371ef76aed83a3b93f0b681e4ccdf37d765b471d9249aee8a0e5c53a6241`。
  - 到期后的真实只读探针补齐了下一自然运行的 fallback 证据：`longbridge check --format json` 显示 CN/Global 端点均连通且独立 CLI session token 为 valid，skill 的 NVDA 结构化 quote 正常返回；全新隔离缓存的 canonical OHLCV router 实测先记录 MCP expired/unavailable，再选择 Longbridge CLI available，统一解析最新交易日为 `2026-07-17`。探针同时发现共享 OHLCV CSV 会让排列靠前的过期 MCP 直接认领 CLI 已写入的缓存，绕过认证和 configured fallback 并污染 vendor provenance。缓存读取现要求窗口内每个交易日的最新 `ohlcv_audit.jsonl` batch vendor 与当前 adapter 完整匹配；来源缺失、混合或属于其他 vendor 一律 cache miss。真实复测得到准确的 MCP → CLI 轨迹，相关扩展门禁 158 项通过；全仓离线门禁为 930 项与 69 个 subtest 通过、3 项依赖缺失/显式 live probe 跳过、2 个已知外网/未隔离用例 deselected。该安全/正确性修复位于 dataflows digest scope，active fingerprint 正确更新为 `bdf2b55bde5b98f9012f562f48b0c4a10a4300a0eaf5b0891209204dd64a6358`，旧无结果 cohort 不得与新 provenance 语义混合。提交推送后的 13:15 CST 正式 timer 新进程以 `already_recorded`、既有 `review_required`、exit 0 完成，未因 fingerprint 更新重复启动同日 Graph。
  ...
  - paired shadow 的实验意图和成本上限现可在首个样本前锁定：启用双臂 schedule 除 `paired_shadow_authorized=true` 外还必须提供 `architecture-experiment-plan/v1`，固定实验 ID、canonical `mean_score_delta` 主指标、20 个最少配对样本、最多配对样本与 canonical `0.002` 最低改善幅度，并把两个架构版本、实际决策 fingerprint 和 RM context treatment 一并规范化为计划 SHA-256 fingerprint。首次正式调度以 create-once、`0600` 文件注册；同 ID 后续改成本上限、换双臂或发生实现/模型配置漂移会 fail closed，达到最大配对样本后返回 `experiment_budget_complete` 并停止新增双倍成本。计划身份同时进入 schedule inventory 和调度结果，dry-run 不产生注册副作用；默认单架构生产 schedule 未启用该能力，automation 层改动不进入决策 implementation digest。针对性架构/评估/调度/CLI/Web 回归 118 项通过；完整离线门禁为 933 项与 69 个 subtest 通过、3 项依赖缺失/显式 live probe 跳过、2 个既有外网/未隔离用例 deselected。使用隔离 SQLite 和正式等价 NVDA schedule 复核仍为 active `bdf2b55bde5b98f9012f562f48b0c4a10a4300a0eaf5b0891209204dd64a6358`、0 terminal/0 outcome、`awaiting_first_active_run`，证明预注册控制面未切分当前生产决策 cohort。
  - 配对实验不再等 5-session outcome 成熟后才发现两次独立 Graph 的上游输入漂移：计划新增 1–5 组小型 pilot（模板默认 2），每个双臂 run 以有界 `architecture-experiment-membership/v1` 固化计划/arm fingerprint 和执行顺序。scheduler 直接从 SQLite terminal event 与 immutable vendor ledger 检查 validated decision、相同非空 market-data date、完整且一致的 RM pre-treatment fingerprint 和 analysis-evidence fingerprint；任一 pair 不可用或不匹配即生成 `architecture-experiment-pilot/v1=failed`，后续两臂返回 `experiment_pilot_failed`/exit 1 并停止新增双倍成本，建议 `implement_shared_pre_treatment_replay`。达到 pilot 数且全部匹配后才继续，之后每个新 pair 仍持续复核。计划和 pilot 经严格白名单进入 CLI/API，Web Evaluations 摘要显示 collecting/passed/failed 与有效进度；API 同时补齐与 CLI 一致的显式 `evaluation_count`，零结果不再依赖调用方从数组推断。默认单臂 schedule 不产生 membership/pilot 副作用，且相关 automation/evaluation/UI 控制面均不进入 decision digest。针对性架构/评估/runtime/调度/CLI/Web 回归 145 项与前端模块通过；完整离线门禁为 936 项与 69 个 subtest 通过、3 项依赖缺失/显式 live probe 跳过、2 个既有外网/未隔离用例 deselected。合成已授权 schedule 的真实 390×844 Chrome 审计为 `scrollWidth=clientWidth=390`、无横向 overflow，长截图确认 `Experiment pilot · Collecting · 0/2` 卡片；正式 Web API 返回 `evaluation_count=0`、active `bdf2b55b…`、plan/pilot 均为空，证明生产仍为未授权单臂且决策 cohort 未切分。
