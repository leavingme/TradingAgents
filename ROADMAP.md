...
- [ ] **7. 连续多日评估与 agent 架构实验门禁**
  ...
  - 提供按架构版本、配置 fingerprint 和 horizon 的 sample count、hit rate、平均 return/alpha/score，以及 runtime/LLM/tool/token 成本与各自覆盖数聚合；LLM/tool/token 还必须按 canonical Agent 归因并保留覆盖数、均值及严格配对差值，同时保留 CLI/API 查询；同名但不同实际配置不得混入同一 rollup。
  ...
  - 运行中 Web API 热加载验证已返回新 schema、固定 20-pair/0.002 改善政策、`not_observed` 完整性与 `continue_sample_collection`，正式库仍为 0 evaluation/0 pending；canonical dry-run fingerprint 保持 `aad8eeaa5998020076078ffbf0d7d0b03bde112ef60f33f463c0574cf1022af8`，证明纯评估控制面改动没有切碎生产决策 cohort。
  - 单架构 rollup 现提供 operator-only 的 `architecture-outcome-assessment/v1`：score 分布/负值风险、raw/alpha 中位数、rating 分层表现，以及按 ticker 与真实 entry/exit 窗口做 Bartlett/Newey-West 重叠校正的 mean-score 95% 区间。少于 20 个样本或 temporal window 不完整会分别标记为 `insufficient_samples` / `incomplete_temporal_evidence`。该字段与成本统计一起从 `include_runtime_costs=False` 的 Agent 纵向上下文排除，针对性 history/runtime/Web/纵向门禁 78 项证明没有在 fingerprint 不变时改变生产 Agent 输入；扩展门禁 351 项、Python 编译和前端语法检查均通过。
