...
分析日收盘价可能早于收盘后决策形成时刻，因此绝不作为可执行 entry。只有完整取得
分析日之后第一个标的/基准共同收盘价，以及再往后第 5 个共同交易日收盘价时，结果
...
当前计量口径为 `next-common-close-v1`，旧版决策日收盘口径明确标记为
...
反思当成可信证据。v4 上下文保留最近的同标的/跨标的逐条结果、计量/评分身份及明确的扫描/截断计数，
