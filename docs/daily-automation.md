...
`architecture-evaluation-status/v4`：它绑定该 run 的 architecture version/fingerprint，
记录最多扫描 5000 条同标的已结算结果后的 pending/blocked/in-progress/cohort 数，以及当前 cohort 的
...
`awaiting_fixed_horizon_outcome`、`settlement_in_progress` 或
`blocked_invalid_history` 状态，并单列安全的
