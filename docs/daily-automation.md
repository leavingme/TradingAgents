...
报告 artifact 的目录、写入和摘要实现位于 architecture implementation digest scope 之外的
`tradingagents.reporting`，runtime 通过稳定的 `write_run_report()` 边界调用。纯报告格式或
存储维护不会建立新的 Agent cohort；决策 runtime、Agent、prompt、validator 和数据输入语义
仍在 digest scope 内，发生变化时必须生成新的 fingerprint。
...
`status` / `evaluate` CLI 和 `/api/evaluations` 还会返回
`scheduled-architecture-inventory/v1`。它复用上述 request/config/manifest 构造，但使用
固定的无副作用日期且只公开安全 identity 字段；不会读取行情、调用 vendor/LLM 或写历史。
`status` 在未到运行窗口时也直接显示下一次调度的 fingerprint；`evaluate` 和 Web 再把
每个 active identity 以 ticker + architecture version + fingerprint 同时匹配终态 run 与
成熟 outcome，状态只可能是等待首次自然运行、运行需要处理、等待结果成熟或已有成熟
结果。Web 会把没有 active identity 匹配的旧 fingerprint 标成 historical，避免旧成本
cohort 被误认作下一次调度的生产基线。schedule 关闭时清单为空并显示 disabled；配置缺失
或无效时 fail closed 为 unavailable，只保留异常类型而不返回文件路径或错误正文。
...
样本截断混入同一均值。token、调用数、工具输入/输出体积和耗时仅用于
operator-facing 架构优化查询，
不会注入投资决策上下文。terminal stats 还会按 canonical Agent 名称归因 LLM calls、
tool calls、input/output tokens，并按 canonical 工具归因输入/输出字符数和错误数。
CLI/API 的 operator 查询以最多 5000 个 `run_id` 回读最终 stats，只复制规范化数值；
单次结果公开 `agent_costs` / `tool_context`，架构 rollup 分别公开 Agent 与工具的覆盖数
和均值，严格配对比较公开 challenger-minus-baseline 的覆盖数、均值、区间以及按先后
执行顺序分层的工具差值。缺失的某一臂成本只计为缺失，不会按零成本参与比较；未知回调元数据
...
固定 5-session outcome 尚未成熟时，CLI/API 还会独立生成
`architecture-run-cost-rollup/v2`：从最多 5000 个最新终态 run 按 ticker、架构版本和
fingerprint 隔离汇总，公开分析日期范围、completed/review_required/unavailable/failed/
cancelled 状态、stats 覆盖、总成本及 Agent/工具热点。`rolling-run-cost-monitoring/v1`
先把同一 ticker/fingerprint/analysis-date 的全部终态尝试相加，再比较最近与紧邻前一组
5/10/20 个分析日的日均 input token 和 runtime；重试成本不会丢失，也不会被当成额外日期。
日期或 token 观测不完整时窗口保持 `insufficient_history`。`run-cost-assessment/v1` 只给出
修复可观测性、继续积累、调查可靠性/近期成本上升或设计受控挑战者的 operator 建议；它
固定 `outcome_claim_allowed=false`、`automatic_architecture_mutation_allowed=false`、
...
`single-architecture-optimization-assessment/v2`，用于在尚未启动 challenger 时判断
...
最高的三个 Agent、平均输出字符最多的三个工具及均分最弱的 rating 分组，并在“继续收集、修复时间证据、修复输入
...
live 顺序固定为：通过结构化行情 readiness → 结算当前已成熟的 SQLite pending outcomes
→ 从 SQLite 重读 canonical v8 context → 构造 Agent state。因此当天刚达到第 5 个后续
共同收盘价的结果，不必等到下一次运行才被 Research/Portfolio 使用；未成熟结果仍保持
pending。历史 `point_in_time` 路径完全跳过事后结算，只能读取其 cutoff 当时已经存在的
结果，避免未来信息进入复现。
若 SQLite 中某条标为 validated 的历史 run 缺少可审计终态决策、5 档 rating、合法日期
或带时区 decision timestamp，结算器不会替它猜测默认值，也不会让它中止当天整个 live
run。该历史 run 会按 run + horizon 写入 `decision_evaluation_issues`，只保存白名单
`issue_code` 与检测/解除时间；结果仍保持 pending 且不能生成 evaluation 或进入 Agent
上下文。CLI 和 `/api/evaluations` 将其显示为 `blocked_invalid_history`，并单列
`blocked_evaluation_count`。若权威历史被修复，下一轮会解除 issue 后重新尝试；SQLite
不可写或 outcome validator 自身失败仍属于系统性故障并继续 fail closed。
结算还必须先取得 SQLite 中 run + horizon 唯一的 `decision_evaluation_claims` 租约。
两个 Web/CLI/scheduler live run 同时看到同一 pending 结果时，只有 owner 可以抓取标的与
基准行情、写 evaluation 和兼容反思；竞争者在构造 Agent state 前抛出类型化
`OutcomeSettlementInProgressError`，不能在成熟结果尚未入库时继续使用旧上下文。owner
正常、未成熟或异常返回都会释放自己的租约；进程硬退出留下的租约一小时后可原子接管，
非 owner 无权释放。CLI/API 将活跃租约显示为 `settlement_in_progress` 并单列
`in_progress_evaluation_count`，只暴露 run identity 与安全 UTC expiry，不保存 provider
正文。
resolver 的空结果只允许表示“标的与基准 OHLCV 均有效，但共同收盘点还不足 entry + 5 个
后续 session”。空 OHLCV、router/vendor 异常以及字段、日期、数值或精确 source ID 校验
失败会在 strict live 路径写入 `decision_evaluation_failures`；记录只包含
`ohlcv_unavailable|outcome_validation_failed`、失败次数和安全 UTC 时间，不复制异常正文。
随后当前 run 以 `OutcomeSettlementDataError` 在 Agent state 构造前 fail closed，避免用缺少
本应成熟结果的上下文继续决策。CLI/API/Web 显示 `retryable_settlement_failure` 并单列
`failed_evaluation_count`；下一次取得有效数据或确认仍未成熟时关闭该失败生命周期。
每日调度器会在 canonical decision 已经形成后追加
`architecture-evaluation-status/v5`：它绑定该 run 的 architecture version/fingerprint，
记录最多扫描 5000 条同标的已结算结果后的 pending/blocked/in-progress/failed/cohort 数，以及当前 cohort 的
outcome status、实验就绪度、建议动作和是否可设计受控实验，并从本轮最终 stats 固化
input token 最高的三个 Agent 与输出字符最多的三个工具。快照反映本轮 Graph 开始时
已完成结算的历史样本；行情仍为 pending 时不生成。该事件不复制逐条 rating、收益、
价格、Prompt、完整成本明细、工具正文或报告内容，适合 SQLite 与 Web 历史回放。它是 operator
...
固定期限 outcome 成熟后，比较器不会仅凭相同版本名和日期推断实验归属。CLI/API 在
operator 查询边界从每个原始 run 的事件链安全附加成员关系；有效 pair 必须属于同一
experiment ID 和同一计划 fingerprint，成员事件中的架构版本/fingerprint 必须与
evaluation 一致，而且声明的 1/2 执行顺序必须与权威 run start timestamp 一致。成员事件
缺失、非法、冲突、跨计划或顺序未实际执行均计入
`experiment_membership_mismatches_excluded`，不能进入收益或成本门禁。同一 cohort 查询中
出现多个预注册计划时，未显式选择计划会返回 `invalid_comparison`，避免把不同实验拼成
更大的样本。
查询方式：
...
venv/bin/python3.12 scripts/daily_analysis.py evaluate --ticker NVDA \
  ...
  --experiment-plan-fingerprint PLAN_SHA256
```
...
当所选两臂与当前 schedule 中预注册计划的两个 arm 完全一致时，Web 会自动附带该计划
fingerprint；不一致的历史 cohort 不会被浏览器强行归入当前计划。比较结果同时显示所选
计划和成员关系排除数。
...
`awaiting_fixed_horizon_outcome`、`settlement_in_progress`、
`retryable_settlement_failure` 或 `blocked_invalid_history` 状态，并单列安全的
`settlement_issue_code`。因此 `evaluation_count=0` 时可以区分“尚无首个决策”“正常等待
固定期限成熟”和“历史证据损坏”，而不必从 Markdown 或日志猜测。
...
离线回归应调用真实 MCP adapter 读取过期 token，并用受控 CLI adapter 证明三条路由；
财务能力必须让 IS/BS/CF/公司资料整体切换并重新通过统一 validator/reconciliation，禁止
拼接不同 vendor 的半套报表。该回归证明 fallback 控制流，不代表真实 CLI OAuth 或网络
当前可用；后者仍需在自然运行前后单独记录 live capability 证据。
...
每次 comparison 还返回固定 schema 的
`architecture-optimization-assessment/v2`，把三类证据分开：
...
及配对差值，`tool_context_hotspots` 展示 baseline 输出字符最多的三个工具、challenger
均值和严格配对差值；完整 `paired_tool_context_by_execution_order` 另用于识别缓存先后
顺序混杂。建议动作只能是继续收集、修复 pair、保留 baseline 或进入人工复核；
