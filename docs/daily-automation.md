...
`evaluate` CLI 和 `/api/evaluations` 还会返回
...
每个 active identity 再以 ticker + architecture version + fingerprint 同时匹配终态 run
与成熟 outcome，状态只可能是等待首次自然运行、运行需要处理、等待结果成熟或已有成熟
