...
`architecture-evaluation-status/v3`：它绑定该 run 的 architecture version/fingerprint，
记录最多扫描 5000 条同标的已结算结果后的 pending/blocked/cohort 数，以及当前 cohort 的
...
`awaiting_fixed_horizon_outcome` 或 `blocked_invalid_history` 状态，并单列安全的
