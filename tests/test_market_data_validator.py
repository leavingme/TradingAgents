...
class TestVerifiedSnapshot:
    def test_load_ohlcv_uses_canonical_three_year_horizon(self, monkeypatch):
        captured = {}

        def routed(method, symbol, start, end):
            captured.update(method=method, symbol=symbol, start=start, end=end)
            return _sample_ohlcv()

        monkeypatch.setattr(validator, "route_to_vendor", routed)
        validator._load_ohlcv("COF", "2026-05-20")
        assert captured == {
    ...
    def test_excludes_future_rows(self, monkeypatch):
        ...
        monkeypatch.setattr(validator, "_load_ohlcv", lambda s, d: data)
    ...
    def test_uses_previous_trading_day_when_date_is_weekend(self, monkeypatch):
        monkeypatch.setattr(validator, "_load_ohlcv", lambda s, d: _sample_ohlcv())
    ...
    def test_raises_on_empty_data(self, monkeypatch):
        monkeypatch.setattr(validator, "_load_ohlcv", lambda s, d: pd.DataFrame())
        with pytest.raises(ValueError):
    ...
    def test_trade_snapshot_requires_matching_audited_market_call(self, monkeypatch):
        from tradingagents.runtime import audit_context, history

        monkeypatch.setattr(validator, "_load_ohlcv", lambda s, d: _sample_ohlcv())
        monkeypatch.setattr(audit_context, "current_run_id", lambda: "run-verified")
        monkeypatch.setattr(history.history_store, "get_vendor_calls", lambda run_id: [{
            "method": "get_stock_data", "selected": 1, "symbol": "COF",
            "call_id": "call-market-1",
        }])
        snapshot, _, _ = validator.build_verified_market_snapshot_data(
            "COF", "2026-05-20"
        )
        assert snapshot.vendor_call_id == "call-market-1"

        monkeypatch.setattr(history.history_store, "get_vendor_calls", lambda run_id: [])
        with pytest.raises(ValueError, match="not bound to an audited vendor call"):
            validator.build_verified_market_snapshot_data("COF", "2026-05-20")
